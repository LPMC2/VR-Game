...
public class PlayerWithRaycastControl : NetworkBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (IsClient && IsOwner)
        {
            if (networkPlayerState.Value == PlayerState.Punch && ActivePunchActionKey())  //get Punchkey from your inputkey
            {
    ...
    private void ClientVisuals()
    {
        if (oldPlayerState != networkPlayerState.Value)
        {
            ...
            {
                animator.SetFloat($"{networkPlayerState.Value}Blend", networkPlayerPunchBlend.Value); //to set Left or Right Punch animation
            }
        }
    }
    ...
    public void UpdateClientPositionAndRotationServerRpc(Vector3 newPosition, Vector3 newRotation)
    {
        ...
        networkRotationDirection.Value = newRotation * Time.deltaTime;
    }
    ...
    {
        var clientWithDamaged = NetworkManager.Singleton.ConnectedClients[clientId]  //tell server to updateHealth of specific client
    ...
    public void UpdatePlayerStateServerRpc(PlayerState state)
    {
        ...
        {
            networkPlayerPunchBlend.Value = Random.Range(0.0f, 1.0f); //random number for Right or Left Punch
        }
    }
